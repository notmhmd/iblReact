...
function useSelect(props) {
  ...
  const optionValues = React7.useMemo(() => Array.from(options.keys()), [options]);
  const getOptionByValue = React7.useCallback((valueToGet) => {
  ...
  const isItemDisabled = React7.useCallback((valueToCheck) => {
    var _option$disabled;
    const option = getOptionByValue(valueToCheck);
    return (_option$disabled = option == null ? void 0 : option.disabled) != null ? _option$disabled : false;
  }, [getOptionByValue]);
  const stringifyOption = React7.useCallback((valueToCheck) => {
    const option = getOptionByValue(valueToCheck);
    if (!option) {
      return "";
    }
  ...
  const controlledState = React7.useMemo(() => ({
    selectedValues: value,
    open: openProp
  }), [value, openProp]);
  const getItemId = React7.useCallback((itemValue) => {
    var _options$get;
    return (_options$get = options.get(itemValue)) == null ? void 0 : _options$get.id;
  }, [options]);
  const handleSelectionChange = React7.useCallback((event, newValues) => {
    if (multiple) {
  ...
  const handleHighlightChange = React7.useCallback((event, newValue) => {
    onHighlightChange == null || onHighlightChange(event, newValue != null ? newValue : null);
  }, [onHighlightChange]);
  const handleStateChange = React7.useCallback((event, field, fieldValue) => {
    if (field === "open") {
  ...
  const useListParameters = {
    ...
    onStateChange: handleStateChange,
    reducerActionContext: React7.useMemo(() => ({
      multiple
    }), [multiple]),
    items: optionValues,
  ...
  const getOptionMetadata = React7.useCallback((optionValue) => getOptionByValue(optionValue), [getOptionByValue]);
  ...
  const contextValue = React7.useMemo(() => _extends({}, listContextValue, compoundComponentContextValue), [listContextValue, compoundComponentContextValue]);
  let selectValue;
  if (props.multiple) {
...
function useMenu(parameters = {}) {
  ...
  const {
    ...
    triggerElement,
    registerPopup
  } = (_React$useContext = React11.useContext(DropdownContext)) != null ? _React$useContext : FALLBACK_MENU_CONTEXT;
  const isInitiallyOpen = React11.useRef(open);
  const {
    subitems,
    contextValue: compoundComponentContextValue
  } = useCompoundParent();
  const subitemKeys = React11.useMemo(() => Array.from(subitems.keys()), [subitems]);
  const getItemDomElement = React11.useCallback((itemId) => {
    var _subitems$get$ref$cur, _subitems$get;
    if (itemId == null) {
      return null;
    }
  ...
  const reducerActionContext = React11.useMemo(() => ({
    listboxRef: rootRef
  }), [rootRef]);
  const {
...
function useMenuItem(params) {
  ...
  const itemMetadata = React14.useMemo(() => ({
    disabled,
    id: id != null ? id : "",
    label,
    ref: itemRef
  ...
  const {
    dispatch
  } = (_React$useContext = React14.useContext(DropdownContext)) != null ? _React$useContext : FALLBACK_MENU_CONTEXT2;
  const {
    getRootProps: getListRootProps,
    highlighted
  } = useListItem({
...
function useMenuItemContextStabilizer(id) {
  ...
  const localGetItemState = React15.useCallback((itemValue) => {
    if (itemValue !== itemId) {
  ...
  const localContextValue = React15.useMemo(() => ({
    dispatch,
...
function useSnackbar(parameters = {}) {
  ...
  const timerAutoHide = React18.useRef();
  React18.useEffect(() => {
    if (!open) {
      return void 0;
    }
...
function useTabs(parameters) {
  ...
  const onSelected = React20.useCallback((event, newValue) => {
    setValue(newValue);
  ...
  const tabIdLookup = React20.useRef(() => void 0);
  const getTabPanelId = React20.useCallback((tabValue) => {
    var _tabPanels$get;
    return (_tabPanels$get = tabPanels.get(tabValue)) == null ? void 0 : _tabPanels$get.id;
  }, [tabPanels]);
  const getTabId = React20.useCallback((tabPanelId) => {
    return tabIdLookup.current(tabPanelId);
  }, []);
  const registerTabIdLookup = React20.useCallback((lookupFunction) => {
...
var Tabs = React23.forwardRef(function Tabs2(props, forwardedRef) {
  var _slots$root;
  const {
    children,
    orientation = "horizontal",
...
function useTabPanel(parameters) {
  ...
  const metadata = React24.useMemo(() => ({
    id,
    ref
  }), [id]);
  const {
...
function useTabsList(parameters) {
  ...
  const tabIdLookup = React25.useCallback((tabValue) => {
    var _subitems$get;
  ...
  const getTabElement = React25.useCallback((tabValue) => {
  ...
  const handleChange = React25.useCallback((event, newValue) => {
    var _newValue$;
    onSelected(event, (_newValue$ = newValue[0]) != null ? _newValue$ : null);
  }, [onSelected]);
  const controlledProps = React25.useMemo(() => {
    if (value === void 0) {
      return {};
    }
    return value != null ? {
  ...
  const isItemDisabled = React25.useCallback((item) => {
  ...
  const contextValue = React25.useMemo(() => _extends({}, compoundComponentContextValue, listContextValue), [compoundComponentContextValue, listContextValue]);
  return {
    contextValue,
    dispatch,
    getRootProps,
...
@mui/base/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-OTXTO5WZ.js.map
